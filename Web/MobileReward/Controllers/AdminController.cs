...
namespace MobileReward.Controllers
{
  ...
  public class AdminController : Controller
  {
    ...
    public ActionResult Users(int page = 1)
    {
      ...
      var userList = db.UserProfiles.Where(x => x.UserId != WebMatrix.WebData.WebSecurity.CurrentUserId).OrderByDescending((x => x.FirstName)).Skip((page - 1) * pageSize).Take(pageSize).ToList();
      ViewBag.CurrentPage = page;
      ViewBag.PageSize = pageSize;
    ...
    public ActionResult Users(string keyword, string command, int page = 1)
    {
      ...
      if (command == "Reset")
      {
        list = db.UserProfiles.Where(x => x.UserId != WebMatrix.WebData.WebSecurity.CurrentUserId).OrderByDescending((x => x.FirstName)).Skip((page - 1) * pageSize).Take(pageSize).ToList();
      }
      else
      {
        list = db.UserProfiles.Where(x =>x.UserId != WebMatrix.WebData.WebSecurity.CurrentUserId && (x.UserName.Contains(keyword) || x.FirstName.Contains(keyword) || x.LastName.Contains(keyword))).OrderByDescending((x => x.FirstName)).Skip((page - 1) * pageSize).Take(pageSize).ToList();
      }
