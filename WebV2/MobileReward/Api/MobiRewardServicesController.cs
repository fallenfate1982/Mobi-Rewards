  ...
  public class MobiRewardServicesController : ApiController
  {
    ...
    public bool Login(string userName, string password)
    {
      try
      {
        ...
        var userStatus = db.UserProfiles.FirstOrDefault(x => x.UserName == userName);
        if (userStatus != null && userStatus.Status == true)
        {
          if (ModelState.IsValid && WebSecurity.Login(userName, password))
          {
            bool exists = Roles.IsUserInRole(userName, "Merchant");
            return exists;
          }
        }
      }
    ...
    /// <returns></returns>
    [HttpGet]
    public bool IsValidCouponByMerchant(string qrcode, string merchantUserName)
    {
      ...
      try
      {
        ...
        var db = new MobiRewardDb();
        var userProfile = db.UserProfiles.FirstOrDefault(x => x.UserName == merchantUserName); 
        var coupon = db.PromotionsCoupon.Where(x => x.QRCode == qrcode && (x.Promotions.UserProfile.UserName == merchantUserName || x.Promotions.UserProfile.UserId == userProfile.ParentId));
    ...
    [HttpGet]
    public CouponDetail GetCouponByIdV2(int id)
    {
      try
      {
        var db = new MobiRewardDb();
        var item = db.PromotionsCoupon.FirstOrDefault(x => x.CouponId == id);
        ...
        string userFile = System.Web.HttpContext.Current.Server.MapPath("~/UserFiles/");
        coupon.CouponImage = GetBytesFromImage(userFile + "Promotions/Coupons/" + (coupon.CouponImageName ?? "nopic.jpg"));
        coupon.BarCodeImage = GetBytesFromImage(userFile + "BarCodes/" + coupon.BarCodeImageName);
        coupon.QRCodeImage = GetBytesFromImage(userFile + "Promotions/Coupons/QRCodes/" + "QR_" + coupon.CouponId + ".png");
    ...
    [HttpGet]
    public List<Promotion> PromotionSearchCountryCode(string promotionName, string endDate, string browseBy, string countryCode)
    {
      try
      {
        var currentDate = DateTime.Now;
        ...
        var q = db.Promotions.Where(x => x.Status && db.PromotionsCoupon.Any(c => c.PromotionId == x.PromotionId          
          && c.Redeemed < c.RedemptionLimit ));       

        if (!String.IsNullOrWhiteSpace(promotionName))
        {
          q = q.Where(x => (x.PromotionName.Contains(promotionName) || x.PromotionTitle.Contains(promotionName) 
            || x.AdvertisementName.Contains(promotionName) 
            || x.PromotionDescription.Contains(promotionName) 
            || (db.PromotionsCoupon.Any(y => y.CouponDescription.Contains(promotionName) 
              || y.CouponName.Contains(promotionName)))));
        }
        ...
        if (!String.IsNullOrWhiteSpace(endDate))
        {
          var tempEndDate = ParseDate(endDate);
          q = q.Where(x =>             
            EntityFunctions.TruncateTime(x.DateStart) <= EntityFunctions.TruncateTime(tempEndDate)            
            && EntityFunctions.TruncateTime(x.DateEnd) >= EntityFunctions.TruncateTime(currentDate)
            );
        }
        else
        {
          q = q.Where(x => EntityFunctions.TruncateTime(x.DateStart) <= EntityFunctions.TruncateTime(currentDate) && EntityFunctions.TruncateTime(x.DateEnd) >= EntityFunctions.TruncateTime(currentDate));
        }

        var p = q.Where(x => (x.MasterCountry.CountryCode == countryCode || x.IsGlobal) && x.IsApproved).ToList();
    ...
    public List<Promotion> PromotionSearch(string promotionName, string endDate, string browseBy, int countryId)
    {
      try
      {
        ...
        {
          var tempEndDate = ParseDate(endDate);
          q = q.Where(x => (EntityFunctions.TruncateTime(x.DateStart) <= EntityFunctions.TruncateTime(currentDate) 
            && EntityFunctions.TruncateTime(x.DateStart) <= EntityFunctions.TruncateTime(tempEndDate)) 
            && (EntityFunctions.TruncateTime(x.DateEnd) >= EntityFunctions.TruncateTime(currentDate) 
            && EntityFunctions.TruncateTime(x.DateEnd) <= EntityFunctions.TruncateTime(tempEndDate)));
        }
        else
        {
    ...
    [HttpGet]
    public List<Promotion> BrowserAllPromotionsByCountryCode(string countryCode)
    {
      try
      {
        var db = new MobiRewardDb();
        ...
        var currentDate = DateTime.Now.Date.Date;
        q = q.Where(x => (x.MasterCountry.CountryCode == countryCode || x.IsGlobal)
                            && EntityFunctions.TruncateTime(x.DateEnd) >= EntityFunctions.TruncateTime(currentDate)
                            && EntityFunctions.TruncateTime(x.DateStart) <= EntityFunctions.TruncateTime(currentDate)
                            && db.PromotionsCoupon.Any(c => c.PromotionId == x.PromotionId
                            && x.IsApproved &&  c.Redeemed < c.RedemptionLimit )
                          );
        ...
        foreach (var item in promotions)
        {
          var coupons = db.PromotionsCoupon.Where(x => x.PromotionId == item.PromotionId 
                  && x.Status);
          if (coupons.Count() == 1)
          {
            item.IsCoupon = true;
