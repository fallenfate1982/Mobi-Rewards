...
namespace MobileReward.Controllers
{
  ...
  public class AccountController : Controller
  {
    ...
    {
      var userStatus = db.UserProfiles.FirstOrDefault(x => x.UserName == model.UserName);
      if (userStatus != null && userStatus.Status == true)
      {
        if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe))
        {
          var controller = "";
          bool exists = Roles.IsUserInRole(model.UserName, "Admin");
          if (exists)
          {
            controller = "Admin";
          }
        ...
        ModelState.AddModelError("", "Invalid username or password (or your account is not activated)");
        return View(model);
      }
      else
      {
    ...
    public ActionResult Register(RegisterModel model)
    {
      if (ModelState.IsValid)
      {
        ...
        try
        {
          ...
          Roles.AddUsersToRole(new[] { model.UserName }, "Merchant");
    ...
    public ActionResult RegisterConfirmation(string Id)
    {
      ...
      {
        //var approved = db.UserProfiles.FirstOrDefault();        
    ...
    [HttpPost]
    public ActionResult ProfileEdit(UserProfile model)
    {
      if (ModelState.IsValid)
      {
        var profile = db.UserProfiles.FirstOrDefault(x => x.UserName == User.Identity.Name);
        var user = db.UserProfiles.Where(x => x.Email == model.Email && x.UserId != profile.UserId);
        if (user.Any())
        {
            ModelState.AddModelError("", "Email already exist!");
            return View(model);
        ...
        if (profile != null)
        {
          ...
          profile.CompanyRegistrationNo = model.CompanyRegistrationNo;
    ...
    public ActionResult ForgetPassword(UserProfile model)
    {
      try
      {
        ...
        var user = db.UserProfiles.FirstOrDefault(x => x.UserName == model.UserName || x.Email == model.UserName);
        ...
        {
          ModelState.AddModelError("", "User name or email id is invalid!");
        }
      }
      catch (Exception ex)
      {
